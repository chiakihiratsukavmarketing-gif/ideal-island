...
- 最新実装コミット: `94e70b6`（World-3 mini）。Vercel反映は push 後
...
5. タスクを完了すると、今日・今月・年間の進捗に反映されます。今日タブで完了すると +1 MILE のトースト（または新規バッジ獲得時は「バッジ獲得」トースト）が表示され、ダッシュボードの MILE 数・ステージ・バッジも更新されます。
...
- Phase Todo-1: 今日のToDoを別日に回す（`dueDate` 更新など最小仕様）
