...
10. 振り返りメモ
11. プロフィール設定（折りたたみ・下部ナビ「設定」で開く）
12. クラウド保存・同期（折りたたみ・summary に状態バッジ）
13. スローガン
...
- プロフィール表示・編集
  ...
  - 設定エリアの「プロフィール設定を開く」（折りたたみ）で表示名・アイコンを編集可能（端末ローカルのみ保存）
...
8. 別端末でも使う場合は、下部ナビの「設定」からプロフィール設定を開き、その下の「クラウド保存・同期を開く」でログインします。ログイン中は変更が約45秒後に自動でクラウド保存されます。必要に応じて「クラウドに保存」「クラウドから読み込み」も使えます。
